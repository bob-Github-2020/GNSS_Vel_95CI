#! /usr/bin/python3
# Last updated: 12-6-2021, by G. Wang
...
# -----------------------------------------------------------------------------
    if output == 'on':
       ...
       y.to_csv(GNSS +'-'+ ENU +'_ACF.txt', index = True, header=True)
       ...
       fig1.text(0.15, 0.9, 'Site velocity: '+ str_bL + '$\pm$' + str_b95CI+' mm/year', ha='center', va='center', transform=fig1.transAxes)
       ...
       fig3.text(0.7,0.07, 'S='+str_a0+str_a1+'cos(2$\pi$x)' + str_b1+'sin(2$\pi$x)'+str_a2+'cos(4$\pi$x)'+str_b2+'sin(4$\pi$x)', ha='center', va='center', transform=fig3.transAxes)
